...
const NoteForm = ({ savedNotes }) => {
  ...
  const deleteNote = async (id) => {
    try {
      ...
      if (result.success) {
        setNotes((prevNotes) =>
          prevNotes.filter((note) => note._id.toString() !== id)
        );
      }
    } catch (error) {
  ...
  const updateNote = async (id, e) => {
    ...
    try {
      ...
      const result = await request.json(); // ✅ FIXED
    ...
    <div className="flex flex-col h-screen w-full bg-[#f5f0eb] font-sans overflow-hidden">
      ...
      <div className="flex-shrink-0 py-4 px-7 border-b-[1.5px] border-[#e0d8cf] bg-[#f5f0eb]">
        <h1 className="font-serif font-bold text-[clamp(1.3rem,3vw,1.9rem)] text-[#2b2218] tracking-tight">
          📓 NoteKeeper{" "}
          <span className="text-sm font-normal text-[#9c8e80] ml-2">
      ...
      <div className="flex flex-1 overflow-hidden flex-col md:flex-row">
        ...
        <div className="flex-shrink-0 w-full md:w-[360px] border-r md:border-r-[1.5px] border-[#e0d8cf] p-7 pb-6 flex flex-col bg-[#faf7f4] overflow-y-auto">
          <label className="text-[0.7rem] font-semibold tracking-wider uppercase text-[#9c8e80] mb-1">
            Title
          </label>
          <input
            type="text"
        ...
        <div className="flex-1 flex flex-col overflow-hidden">
          <div className="flex-shrink-0 flex items-baseline gap-2 px-7 py-5 border-b-[1.5px] border-[#e0d8cf] bg-[#f5f0eb]">
            <h2 className="font-serif font-semibold text-[clamp(1rem,2.5vw,1.35rem)] text-[#2b2218]">
              All Notes
            </h2>
            <span className="text-[0.78rem] text-[#9c8e80] bg-[#e8e0d6] px-2 py-[2px] rounded-full">
              {notes.length}
            </span>
          </div>
            ...
            {notes.length === 0 ? (
              ...
              <div className="grid gap-4 grid-cols-1 sm:grid-cols-2 xl:grid-cols-3">
                {notes.map((note) => (
                  <div
                    key={note._id}
                    className="bg-white border-[1.5px] border-[#e8e0d6] rounded-2xl p-4 flex flex-col gap-2"
                  >
                    <div className="font-serif font-semibold text-[#2b2218] text-base truncate">
                      {note.title}
                    </div>
                    <div className="text-sm text-[#7a6e64] line-clamp-3 flex-1">
                      {note.content}
                    </div>
                    ...
                    <div className="flex gap-2 mt-1">
                      <button
                        ...
                        className="flex-1 py-1 bg-[#eef4ff] text-[#3b6fd4] text-xs rounded-lg"
                      >
                        Edit
                      </button>
                      <button
                        onClick={() => deleteNote(note._id)}
                        className="flex-1 py-1 bg-[#fff0f0] text-[#d94f4f] text-xs rounded-lg"
                      >
                        Delete
                      </button>
                    </div>
